...
exports.updateContract = async (req, res) => {
  try {
    ...
    await existingContract.update({
      ...
      ongoingdate: new Date().toISOString(),
    });
...
exports.getAllInvoicesByManufacture = async (req, res) => {
  try {
    ...
    // Fetch the party details using the provided partyId
    const party = await userTable.findByPk(partyId);

    // If the party does not exist, return a 404 error
    if (!party) {
      return res.status(404).json({ error: "Party not found" });
    }

    // Determine the 'under' value based on the party's userTypeId
    let under = null;
    if (party.userTypeId === 4) {
      under = partyId;
    } else if (party.userTypeId === 8) {
    ...
    // Fetch invoices with related contract and party details
    const invoices = await Invoice.findAll({
      include: [
        {
          model: Contract,
          ...
          include: [
            {
              model: userTable,
              as: "partyuser",
              attributes: ["id", "name"], // Add attributes you want to retrieve
            },
          ],
          where: { under: under },
        },
