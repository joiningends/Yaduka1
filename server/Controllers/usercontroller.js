...
exports.createUser = async (req, res) => {
  ...
  try {
    const { name, mobileNumber, email, address, terminate, companyname } =
      req.body;
    ...
    const existingUser = await userTable.findOne({
      where: { mobileNumber }, // Check if the mobile number matches any existing user
    });

    if (existingUser) {
      // If a user with the same mobile number exists, send an error response
...
exports.getPartiesByUserIds = async (req, res) => {
  try {
    ...
    const userResults = await userTable.findAll({
      where: {
        [Sequelize.Op.and]: [{ userTypeId: user2Id }, { under: userId }],
      },
    });
    ...
    const uniqueResults = Array.from(
      new Set(combinedResults.map(result => result.mobileNumber))
    ).map(mobileNumber => {
      return combinedResults.find(
        result => result.mobileNumber === mobileNumber
      );
    });
...
exports.getAllPartiesAndUsers = async (req, res) => {
  try {
    ...
    const partiesWithoutMatchingUser = parties.filter(
      party => !users.some(user => user.mobileNumber === party.mobileNumber)
    );
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching parties and users." });
  }
};
      ...
      include: [
        {
          ...
          as: "party",
          where: {
            isActive: true, // Filter associated users by isActive field
          },
        },
      ],
    });
      ...
      include: [
        {
          ...
          attributes: [
            "id",
            [
              sequelize.fn("MAX", sequelize.col("partyuser.name")),
              "partyuser_name",
            ],
            [
              sequelize.fn("MAX", sequelize.col("partyuser.mobileNumber")),
              "partyuser_mobileNumber",
            ],
            [
              sequelize.fn("MAX", sequelize.col("partyuser.companyname")),
              "partyuser_companyname",
            ],
          ],
          allowNull: true,
        },
...
exports.getUserByIda = async (req, res) => {
  const userId = req.params.id;
...
exports.getUsersUnderIdmanufacture = async (req, res) => {
  const userId = req.params.id;
